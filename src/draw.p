...
draw_text :: (renderer: *Renderer, theme: *Theme, text: string, x: s32, y: s32) {
    set_blending(.Normal);
    ...
    render_tinted_text_with_font(*theme.font, text, x, y, .Left, theme.font_color.r, theme.font_color.b, theme.font_color.b, xx draw_single_glyph, xx renderer);
}
...
draw_text_input :: (renderer: *Renderer, theme: *Theme, input: *Text_Input, x: s32, y: s32) {
    ...
    if input.active {
        ...
        draw_text(renderer, theme, input.tool_tip, x, y);
    }
}

draw_backlog :: (renderer: *Renderer, theme: *Theme, backlog: *[..]string, x: s32, y: s32) {
    index: s64 = backlog.count - 1;
    while y > 0 && index >= 0 {
        log := array_get(backlog, index);
        draw_text(renderer, theme, ~log, x, y);
        y -= theme.font.line_height;
        --index;
    }
}
